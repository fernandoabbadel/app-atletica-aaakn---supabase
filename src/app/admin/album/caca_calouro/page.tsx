...
export default function AdminAlbumCacaCalouroPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!mounted) return;
        setRows([...ranking].sort((a, b) => (b.scansT8 || 0) - (a.scansT8 || 0)));
      } finally {
    ...
    <div className="min-h-screen bg-[#050505] text-white font-sans pb-20">
      <header className="sticky top-0 z-20 bg-[#050505]/90 backdrop-blur-md border-b border-zinc-800 px-6 py-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black uppercase tracking-tight">Caca Calouro</h1>
            <p className="text-[11px] text-zinc-500 font-bold">Ranking por scans T8</p>
          </div>
        </div>
      </header>
      ...
      <main className="px-6 py-6 max-w-4xl mx-auto space-y-3">
        {loading ? (
          ...
          <div className="text-sm text-zinc-500 border border-zinc-800 rounded-xl p-5">Sem dados.</div>
        ) : (
          paged.map((row, idx) => (
            <article key={row.id} className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 flex items-center gap-3">
              ...
              <div className="text-right text-yellow-400 font-black text-sm inline-flex items-center gap-1">
                <ShieldCheck size={14} /> {row.scansT8 || 0}
              </div>
            </article>
          ))
        )}
