...
export function LeaguePublicDetailClient({
  ...
  const tenantPath = (path: string) => (tenantSlug.trim() ? withTenantSlug(tenantSlug, path) : path);
  ...
  useEffect(() => {
    ...
    const run = async () => {
      setLoading(true);
      try {
        const nextLeague = await fetchLeagueById(leagueId, {
          forceRefresh: true,
    ...
    return (
      <div className="min-h-screen bg-[#050505] px-6 py-10 text-white">
        <div className="mx-auto max-w-3xl rounded-[2rem] border border-zinc-800 bg-zinc-950/80 p-8 text-center">
          <p className="text-[10px] font-black uppercase tracking-[0.28em] text-zinc-500">Liga nao encontrada</p>
          <h1 className="mt-4 text-3xl font-black uppercase tracking-tight text-white">Essa pagina nao esta disponivel</h1>
          <p className="mt-3 text-sm leading-6 text-zinc-400">A liga pode ter sido removida ou ainda nao estar visivel neste tenant.</p>
    ...
    <div className="min-h-screen bg-[#050505] pb-20 font-sans text-white">
            ...
            <div className="mt-6 grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
              <div className="flex flex-col gap-5 sm:flex-row sm:items-start">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-[10px] font-black uppercase tracking-[0.28em] text-emerald-400">Ecossistema academico</p>
                  <h1 className="mt-3 text-3xl font-black uppercase tracking-tight text-white sm:text-4xl">{league.nome}</h1>
                  <p className="mt-3 max-w-2xl text-sm leading-6 text-zinc-300 sm:text-base">{league.descricao || "Liga oficial com pagina propria para mostrar membros, agenda e identidade visual."}</p>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <button type="button" onClick={() => void handleLike()} disabled={!user} className={`rounded-[1.5rem] border p-4 text-left transition disabled:cursor-not-allowed disabled:opacity-60 ${isLiked ? "border-red-500/30 bg-red-500/10 text-red-100" : "border-white/10 bg-white/5 text-zinc-100 hover:border-red-500/30 hover:bg-red-500/10"}`}>
                  ...
                  <p className="mt-4 text-sm font-bold">{user ? "Mostrar que voce curtiu a liga" : "Entre para curtir"}</p>
                </button>
      ...
      <main className="mx-auto max-w-6xl space-y-6 px-6 py-2">
        <nav className="flex flex-wrap gap-3">
          {[
            { href: overviewHref, label: "Visao geral", tab: "overview" as const },
        ...
        {activeTab === "overview" ? (
          <div className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
            <section className="space-y-6">
              <article className="rounded-[2rem] border border-white/10 bg-[linear-gradient(180deg,rgba(22,22,22,0.96),rgba(10,10,10,0.98))] p-6 shadow-[0_24px_70px_rgba(0,0,0,0.32)]">
                ...
                <p className="mt-4 text-sm leading-7 text-zinc-300">{league.descricao || "Essa liga ainda nao publicou um texto de apresentacao completo."}</p>
              ...
              <article className="rounded-[2rem] border border-white/10 bg-[linear-gradient(180deg,rgba(22,22,22,0.96),rgba(10,10,10,0.98))] p-6 shadow-[0_24px_70px_rgba(0,0,0,0.32)]">
                <div className="flex items-center justify-between gap-3">
                  <div><p className="text-[10px] font-black uppercase tracking-[0.24em] text-zinc-500">Membros em destaque</p><h2 className="mt-3 text-2xl font-black text-white">Liderancas da liga</h2></div>
            ...
            <section className="space-y-6">
              <article className="rounded-[2rem] border border-white/10 bg-[linear-gradient(180deg,rgba(11,18,24,0.96),rgba(10,10,10,0.98))] p-6 shadow-[0_24px_70px_rgba(0,0,0,0.32)]">
                <div className="flex items-center gap-3"><div className="rounded-2xl border border-cyan-500/20 bg-cyan-500/10 p-3 text-cyan-200"><CalendarDays size={18} /></div><div><p className="text-[10px] font-black uppercase tracking-[0.24em] text-cyan-300">Agenda da liga</p><h2 className="mt-2 text-2xl font-black text-white">O que vem por ai</h2></div></div>
          ...
          <section className="space-y-5">
            <div className="rounded-[2rem] border border-white/10 bg-[linear-gradient(180deg,rgba(22,22,22,0.96),rgba(10,10,10,0.98))] p-6 shadow-[0_24px_70px_rgba(0,0,0,0.32)]">
              ...
              <h2 className="mt-3 text-2xl font-black text-white">Ordem de importancia da gestao</h2>
              <p className="mt-3 text-sm leading-7 text-zinc-400">Presidente, vice-presidencia, secretaria, tesouraria, diretoria e membros.</p>
            </div>
          ...
          <section className="space-y-5">
            <div className="rounded-[2rem] border border-white/10 bg-[linear-gradient(180deg,rgba(11,18,24,0.96),rgba(10,10,10,0.98))] p-6 shadow-[0_24px_70px_rgba(0,0,0,0.32)]">
              ...
              <h2 className="mt-3 text-2xl font-black text-white">Eventos, encontros e convocacoes</h2>
              <p className="mt-3 text-sm leading-7 text-zinc-400">Tudo que a liga publicou para a comunidade acompanhar em um so lugar.</p>
            </div>
            <div className="grid gap-4 lg:grid-cols-2">
              ...
              {sortedEvents.length === 0 ? <p className="rounded-[1.75rem] border border-dashed border-zinc-800 bg-zinc-950/70 p-8 text-center text-sm text-zinc-500">A agenda da liga ainda esta vazia.</p> : null}
            </div>
          </section>
        )}
